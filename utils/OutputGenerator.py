...
class OutputGenerator:
    ...
    def _embed_data(self):
        ...
        # Replace backslashes, backticks, and dollar signs
...
window.__ACCOUNT_ID__ = "{self.account_id}";
    ...
    def _extract_modernize_from_json(self):
        ...
        try:
            ...
            # Modernize data structure: {Computes: {nodes: [], links: []}, Databases: {...}}
            modernize_data = {}
            ...
            for file_path in modernize_files:
                try:
                    with open(file_path, 'r') as f:
                        data = json.load(f)
                        # Merge data from all service files
            ...
            _info(f"Extracted Modernize data with keys: {list(modernize_data.keys())}")
    ...
    def _extract_ta_data(self):
        """
        Extract Trusted Advisor data from CustomPage.TA.*.json files.
        ...
        try:
            ...
            if not ta_files:
                ...
                return {'error': 'No Trusted Advisor data available'}
            ...
            if 'error' in ta_data or not ta_data:
                _info("TA data contains error or is empty")
                return ta_data if ta_data else {'error': 'No Trusted Advisor data available'}
            
            _info(f"Extracted TA data with pillars: {list(ta_data.keys())}")
            ...
            import traceback
            traceback.print_exc()
            return {'error': f'Failed to load Trusted Advisor data: {str(e)}'}
